...
# 🧠 HANDOFF DOCUMENT — DeskLoom v0.5 Phase 9-6 (Complete)
## ส่งต่อให้ Phase 10
...
## ✅ สิ่งที่เสร็จแล้วใน Phase 9-6 (ทั้งหมด)

### Lesson 1 — tauri.conf.json: Hidden Main Window ✅
- Main window เพิ่ม `"visible": false` + `"skipTaskbar": true`
- Window ซ่อนตั้งแต่ start WebView ยังทำงาน Zustand store ยังรัน

### Lesson 2 — lib.rs: Tray Menu ใหม่ ✅
- Menu เปลี่ยนเป็น: Settings / Focus Mode / Quit
- Left-click tray: ถ้า window visible → hide, ถ้า hidden → emit `tray-open-settings` + show + focus
- "Settings" menu: emit `tray-open-settings` + show + focus
- "Focus Mode" menu: emit `tray-toggle-focus` (ไม่ show window)

### Lesson 3 — App.tsx: Tray Event Listeners ✅
- เพิ่ม `listen("tray-open-settings")` → `setIsSettingsOpen(true)`
- เพิ่ม `listen("tray-toggle-focus")` → `toggleFocusMode()`
- `handleCloseSettings` → `setIsSettingsOpen(false)` + `getCurrentWindow().hide()`
- SettingsPanel ใช้ `onClose={handleCloseSettings}`
...
## ✅ Phase 9 เสร็จสมบูรณ์ทั้งหมด

Phase 9 ครอบคลุม:
- 9-1: Per-widget window (OS window แยกแต่ละ widget)
- 9-2: Widget window settings (opacity, click-through ต่อ widget)
- 9-3: Quick Capture window
- 9-4: Multi-monitor support
- 9-5: Always-on-top per widget + drag lag fix
- 9-6: Tray controller (hidden main window) ← **จบที่นี่**

**git tag:** `v0.5.0` (ทำหลัง build ผ่าน)
...
### เป้าหมาย Phase 10
เตรียมแอปสำหรับ distribution จริง

### สิ่งที่ต้องทำ

#### 10-1: Tauri Bundle / Installer
- `tauri build` → `.msi` installer สำหรับ Windows
- ตั้งค่า bundle metadata ใน `tauri.conf.json` (productName, identifier, icon)
- Test install บน Windows จาก .msi

#### 10-2: Onboarding Flow ปรับปรุง
- OnboardingOverlay ปัจจุบัน basic มาก — ปรับให้ user-friendly
- แนะนำ tray icon (เพราะ main window ซ่อนแล้ว user อาจงง)
- แสดง tray icon location เมื่อ first run

#### 10-3: Weather API Key UI
- Widget weather ต้องการ API key จาก OpenWeatherMap
- เพิ่ม input field ใน SettingsPanel สำหรับ API key
- เก็บ key ใน store + save ลง disk (อย่า hardcode)
...
อ่าน HANDOFF นี้ให้ครบ แล้วสรุปสถานะ 1 ย่อหน้า จากนั้นเริ่ม Phase 10 ได้เลย
