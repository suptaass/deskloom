...
// WIDGET_TYPE_ICON — derive จาก registry แทน hardcode
// เพิ่ม widget type ใหม่ใน registry → ไอคอนปรากฏอัตโนมัติที่นี่
...
interface SettingsPanelProps {
  ...
  autostart: boolean;
  alwaysOnTop: boolean;
  /** entries ที่เพิ่มได้ (isProtected = false) — ส่งมาจาก App.tsx */
  ...
  /** รับ WidgetType ทุกตัว — guard ป้องกัน protected อยู่ใน App.tsx แล้ว */
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  return (
    <div style={panelStyle}>
      ...
      {/* Add Widgets — registry-driven: ไม่ต้องแก้ไฟล์นี้อีกเมื่อเพิ่ม widget ใหม่ */}
      <div style={sectionStyle}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          {addableEntries.map((entry) => (
            <button
              ...
              onClick={() => onAddWidgetInstance(entry.type)}
            >
              + {entry.displayName}
            </button>
          ))}
